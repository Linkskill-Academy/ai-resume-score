...
# ai_resume_score_app.py (v1.2)
# LinkSkill Academy — AI Resume Score QR Booth (Streamlit)
# Adds: file-size limit, distinct pre-check statuses, clearer error handling.
...
MAX_FILE_MB  = 5  # adjust if needed
MAX_FILE_BYTES = MAX_FILE_MB * 1024 * 1024

# -----------------------------
# ROLE WORDS / HEURISTICS
# -----------------------------
ROLE_KEYWORDS = {
    "General / Fresher": [
...
def precheck_file(file):
    """Return dict of checks + first_error (or '')."""
    name = file.name
    ...
    if not checks["type_supported"]:
        first_error = "Unsupported file type. Please upload PDF, DOCX or TXT."
        return checks, first_error

    if not checks["not_empty"]:
        first_error = "This file is empty. Please upload a valid resume file."
        return checks, first_error

    if not checks["size_ok"]:
        first_error = f"File too large ({fmt_mb(size)}). Max allowed is {MAX_FILE_MB} MB."
        return checks, first_error

    # Soft-read a little to detect encrypted/unreadable PDFs
    try:
        data = file.read()
        file.seek(0)
            ...
            try:
                ...
                if reader.is_encrypted:
                    try:
                        ...
                        first_error = "This PDF is password-protected. Please export an unlocked PDF or upload DOCX."
                        return checks, first_error
                txt = ""
                ...
                if not checks["readable_text"]:
                    first_error = "This PDF looks like a scanned image. Please upload a text-based PDF or DOCX."
                    return checks, first_error
            except Exception:
                first_error = "We couldn't open this PDF. Please export a new text-based PDF or upload DOCX."
                return checks, first_error
        elif ext == ".docx" and docx2txt is not None:
            # we'll try full read in extractor later; mark as text-capable
            checks["readable_text"] = True
        elif ext == ".txt":
            checks["readable_text"] = True
    except Exception:
        first_error = "We couldn't read this file. Please re-upload or try a different format."
        return checks, first_error

    return checks, ""

def extract_text_from_file(uploaded_file) -> (str, str):
    """Return (text, error_hint). error_hint is empty string if OK."""
        ...
        try:
            ...
            if reader.is_encrypted:
                try:
                    ...
                    return "", "This PDF is password-protected. Please export an unlocked PDF or upload DOCX."
            ...
            return "", "We couldn't open this PDF. Please export a new text-based PDF or upload DOCX."
        if not text.strip():
            return "", "This PDF looks like a scanned image. Please upload a text-based PDF or DOCX."
        return text, ""
    # DOCX
    if name.endswith(".docx"):
        ...
        try:
            ...
            try:
                ...
                pass
            if not text.strip():
                return "", "We couldn't read text from this DOCX. Please save again or upload as PDF."
            return text, ""
        except Exception:
            return "", "We couldn't open this DOCX. Please save again or upload as PDF."
    # TXT
...
st.caption(f"Allowed types: **PDF/DOCX/TXT** • Max size: **{MAX_FILE_MB} MB** • Tip: If the file picker doesn't open in Instagram/LinkedIn, use **Open in Chrome/Safari**.")
...
if submitted:
    if not (name and email and phone and uploaded and agree):
        ...
        # Prechecks with distinct status
        checks, first_error = precheck_file(uploaded)
        st.markdown("#### Pre-checks")
        col_a, col_b, col_c, col_d, col_e = st.columns(5)
        col_a.write(("✅" if checks["type_supported"] else "❌") + " Type supported")
        size_txt = f"{fmt_mb(getattr(uploaded, 'size', 0))} / {MAX_FILE_MB} MB"
        col_b.write(("✅" if checks["size_ok"] else "❌") + f" Size OK ({size_txt})")
        col_c.write(("✅" if checks["not_empty"] else "❌") + " Not empty")
        col_d.write(("✅" if checks["not_password_protected"] else "❌") + " Not password-protected")
        col_e.write(("✅" if checks["readable_text"] else "⚠️") + " Text is readable")
        ...
        if first_error:
            ...
            with st.spinner("Analyzing your resume..."):
                ...
                if hint:
                    ...
                    st.markdown("#### 📊 Details")
                    st.write(f"Word Count: **{breakdown['Word Count']}**")
